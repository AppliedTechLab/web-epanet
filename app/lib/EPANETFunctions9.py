...
from EPANETClasses9 import * #get all classes from EPANETClasses file
...
def ReadEPANET(filename):
    ...
    patternsList=CopySection("PATTERNS",fullcontentList)
    #optionsList=CopySection("OPTIONS",fullcontentList)

    coordinatesList=CopySection("COORDINATES",fullcontentList)
    emittersList=CopySection("EMITTERS",fullcontentList)
    demandsList=CopySection("DEMANDS",fullcontentList)
    tagsList=CopySection("TAGS",fullcontentList)
    ...
    emitters=[]
    i=0
    while i < len(emittersList):
        e=emittersList[i].split()
        numElem=len(e)
        if numElem<2:
            print('Warning! missing emitter elements!')
            ID='err'
            coef=0
        else:
            ID=e[0]
        ...
        emitters.append(Emitter(ID,coef,colonYN))
        i+=1
    ...
    while i < len(tagsList):
        ...
        if numElem==3:
            ...
            print('Warning! emitter list is missing an element')
            t=''
            ID=''
            tag=''
        if e[-1][-1]==';':
        ...
        tags.append(Tag(t,ID,tag,colonYN))
        i+=1
    
    patterns=[]
    
    i=0
    while i < len(patternsList):
        #print("found a pattern")
        e=patternsList[i].split()
        #print(e)
        numElem=len(e)
        if numElem>1:
            ID=e[0]
            ...
            multipliers=map(float,multipliers)
            #print multipliers
            #print ID
            #print colonYN
            #print multipliers
            #print(DemPattern(ID,multipliers,colonYN))
            patterns.append(DemPattern(ID,multipliers,colonYN))

        #else:
        #    print('WARNING, No patterns found in pattern section')

        
        i+=1
    #Now we need to merge patterns with the same name
    #print(patterns)
    uPatterns=[]
    patNames=[]
    for p in patterns:
        patNames.append(p.ID)
    uPatNames = list(set(patNames))
    #print uPatNames
    for uP in uPatNames:
        allMults=[]
        colonYN=False
        for p in patterns:
            if p.ID==uP:
                thisMult=p.Mult
                colonYN=p.Colon
                allMults=[allMults,thisMult]
                allMults=[m for tM in allMults for m in tM] #flatten list
        uPatterns.append(DemPattern(uP,allMults,colonYN))
    #print uPatterns
    #print demands
    returnDict = {'FullContent':fullcontentList,'Junctions':junctions,'Tanks':tanks,'Pipes':pipes,'Coordinates':coordinates,'Demands':demands,'Emitters':emitters,'Tags':tags,'Patterns':uPatterns}
    #print(returnDict)    
    #return{1}
    return returnDict


def WriteEPANET(filenameFIX, junctionsFIX, tanksFIX, pipesFIX, coordinatesFIX, demandsFIX,emittersFIX,tagsFIX, fullcontentListFIX):
    ...
    emittersList_adj=Collate(emitters)
    tagsList_adj=Collate(tags)
    ...
    fullcontentList=Splice("TAGS",fullcontentList,tagsList_adj)
    fullcontentList=Splice("DEMANDS",fullcontentList,demandsList_adj)
    fullcontentList=Splice("EMITTERS",fullcontentList,emittersList_adj)
...
#            if coordinates[j].ID == junctions[i].ID:
...
def MakeSpecifiedSumps( junctions,tanks,pipes,coordinates, coordOff,HHDaily, sDepth, bpDepth,sList,bpList):
    ...
    #Visual offest
    xoff=coordOff #25  #these values give the tanks their offest on the map
    yoff=coordOff#25
    ...
    i=0
    while i < len(junctions):
        if junctions[i].Demand > 0: #then there is a demand at this node! And this skips sources!
    ...
    for i in fixlist:   #i is now the indext of things to fix. call the right function!
        ...
        #join with adequate pipes
        pipes.append(Pipe("B"+str(k),junctions[i].ID,tanks[-1].ID,pipeLengthHH,newSizes['newPipeDia'],pipeRoughHH,newSizes['newLossCoeff'],"CV",pcolon))
